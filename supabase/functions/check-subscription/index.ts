...
serve(async (req) => {
  ...
  try {
    ...
    if (userError) {
      logStep("Auth error - session may be expired", { error: userError.message });
    ...
    if (hasActiveSub) {
      ...
      const { data: orgData } = await supabaseClient.rpc('get_user_organization_id', { _user_id: user.id });
      
      if (orgData) {
        // Determine plan from product ID
        let plan = 'starter';
        if (productId === 'prod_TmEZZY5hzUCPhe') {
          plan = 'professional';
        }
